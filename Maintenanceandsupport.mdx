---
title: "Maintenance and Support"
description: "This section outlines Kontratar’s commitment to system reliability, security, and user assistance. It details the ongoing updates, technical support, and troubleshooting services available to ensure uninterrupted platform performance and a smooth user experience."
---

### **Technical Support**

- **For general inquiries:**

  ![Image(3) Pn](/images/image\(3\).png)
  - Click on the **Support** icon at the bottom right of your screen.
  - Reach out to us via phone or email.

  **For issues with the Kontratar application:**
  - Click on **Feedback** in the sidebar or from the menu.
  - Select the **\+** icon to create a new ticket.
  - Provide detailed information about the issue and submit your request.
  - The newly created ticket will appear in your ticket list.
  - Click on the ticket to add additional details or comments.
  - A member of our team will communicate with you through each ticket you raise until the issue is resolved.

<Note>
  If you have previously reported an issue, please continue communication within the existing ticket for related cases rather than opening a new one.
</Note>

### For issues with the Kontratar application:

- Click on the support icon on the bottom right side of your screen.
- Click on **Raise an Issue**.

  ![Image(7) Web](/images/image\(7\).webp)

  ![Image(8) Web](/images/image\(8\).webp)
- Select the **\+** icon on your page.
- Provide detailed information about the issue and submit your request.
- You will see the ticket you just created.
- Click on the ticket to add more details or complaints.
- A member of our team will communicate with you via each ticket you raise.
- Once the issue is resolved and confirmed on both ends:
  - We will close the ticket on our end, and further communication on that ticket will no longer be possible.
  - If you are satisfied with our response, you may also close the ticket yourself.
- Feel free to open as many tickets as needed.
- However, if you have previously reported an issue, please continue communication on the existing ticket for related cases instead of opening a new one.

### For Feature Requests

- **To submit a new feature request:**
  - Go to the **Feedback** page.
  - Click on **Feature Request**.
  - Select the **\+** icon to create a new ticket.
  - Provide a detailed explanation of the feature you would like to see, including its purpose and potential benefits.
  - Submit your request.
  - The newly created ticket will appear in your ticket list.
  - Click on the ticket to add more details or feedback.
  - A member of our team will communicate with you through each ticket you raise until the request has been reviewed.

<Note>
  If you have previously submitted a feature request, please continue communication within the existing ticket for related updates rather than opening a new one.
</Note>